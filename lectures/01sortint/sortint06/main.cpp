...
// in standard library we have containers and iterators (abstractions that provide access to every kind of container)

// replace mdp:: with std:: to work with the standard library implementation


// DO NOT USE using namespace AT FILE LEVEL, TRY TO USE IT EVERYWHERE IN RETURN TYPES AND FUNCTION DECLARATION, INSIDE FUNCTIONS DO WHATEVER YOU WANT
...
namespace mdp { //namespace useful when we have classes named the same but different
	template<typename T>
...
// this print only works for vectors because we can access every element by indexing (using [] operator)
// for lists it would be inefficient because we would access all the elements up to the i-th 
// with dictionary it wouldn't work 
/*
void print(FILE* f, const mdp::vector<int>& v) { // if we use vector v numbers and v are two different variables (different addresses in memory) but point to the same location (data)
...
*/
//uniform system to access containers -> iterators 
void print(FILE* f, const std::vector<int>& v) { // if we use vector v numbers and v are two different variables (different addresses in memory) but point to the same location (data)

	//const int* start = &v[0];
	//iterator specific for every type of the same kind of object
	std::vector<int>::const_iterator start = v.begin(); // begin method available in any container, it provides an iterator to the first element 
	//const int* stop = start + v.size();
	std::vector<int>::const_iterator stop = v.end();
	//const int* p = start; 
	std::vector<int>::const_iterator it = start;

	//to avoid writing all the type for iterator we can ask compiler to put the type (compiler knows it)
	//auto start = v.begin(); 
	//auto stop = v.end();
	//auto it = start;
	/*
	while (p != stop) {
		int x = *p; // p->elem in a list
		fprintf(f, "%d\n", x);
		++p; // If we change it to p = p->next it would work also for list, somehow we could also make it work for trees (harder)
	}
	*/
	while (it != stop) { // iterators are classes defined within each container that redefine the *, != and increment operators
		int x = *it;
		//auto x = *it; with the type inferred from the compiler we achieved a general function that works for every container, with auto we are calling the copy constructor
		//const auto& x = *it // by taking a reference we don't call the copy constructor -> Much more efficient, use const reference as much as you can
		fprintf(f, "%d\n", x);
		++it;
	}
	...
	/*
	* FOR LOOP ITERATOR
	* begin() and end() are generic functions that call the .begin() and .end() methods if they exist
	* using begin() and end() functions would work also with arrays
	for(auto it = begin(v), stop = end(v); it != stop; ++it){ //if we put it != v.end() we evaluate it every time (inefficient)
		const auto& x = *it;
		fprintf(f, "%d\n", x);
	}
...
std::vector<int> read(FILE* f) {
	...
	//using mdp::vector; //import in these brackets the name mdp::vector such that we can use only vector and the compiler knows we are referring to mdp::vector
	//using namespace mdp to import everything in the namespace (in case we have multiple classes)
	using std::vector;
	if (f == nullptr) {
...
int main(int argc, char** argv) {
	//size_t x = sizeof(vector);
	{
		...
		// Non elegant version to work with standard vector 
		//qsort( &numbers[0] , numbers.size(), sizeof(int), compare); //DO NOT WRITE &numbers (every class is an object that manages memory, memory is not in the first position)
		// Elegant version
